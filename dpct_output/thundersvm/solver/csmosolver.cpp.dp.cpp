...
CSMOSolver::solve(const KernelMatrix &k_mat, const SyncArray<int> &y, SyncArray<float_type> &alpha, float_type &rho,
    ...
    int max_iter =
        std::max(100000, ws_size > INT_MAX / 100 ? INT_MAX : 100 * ws_size);
...
CSMOSolver::calculate_rho(const SyncArray<float_type> &f_val, const SyncArray<int> &y, SyncArray<float_type> &alpha,
    ...
    for (int i = 0; i < alpha.size(); ++i) {
        ...
        if (is_I_up(alpha_data[i], y_data[i], Cp, Cn)) up_value =
            fmin(up_value, f_val_data[i]);
        if (is_I_low(alpha_data[i], y_data[i], Cp, Cn)) low_value =
            fmax(low_value, f_val_data[i]);
    }
