...
NuSMOSolver::calculate_rho(const SyncArray<float_type> &f_val, const SyncArray<int> &y, SyncArray<float_type> &alpha,
    ...
    for (int i = 0; i < alpha.size(); ++i) {
        if (y_data[i] > 0) {
            ...
            if (is_I_up(alpha_data[i], y_data[i], Cp, Cn)) up_value_p =
                fmin(up_value_p, -f_val_data[i]);
            if (is_I_low(alpha_data[i], y_data[i], Cp, Cn)) low_value_p =
                fmax(low_value_p, -f_val_data[i]);
        } else {
            if (alpha_data[i] > 0 && alpha_data[i] < Cn) {
                n_free_n++;
                sum_free_n += -f_val_data[i];
            }
            if (is_I_up(alpha_data[i], y_data[i], Cp, Cn)) up_value_n =
                fmin(up_value_n, -f_val_data[i]);
            if (is_I_low(alpha_data[i], y_data[i], Cp, Cn)) low_value_n =
                fmax(low_value_n, -f_val_data[i]);
        }
    }
