...
int main(int argc, char **argv) try {
    try {
        ...
        if (parser.param_cmd.svm_type == SvmParam::NU_SVC) {
            ...
            for (int i = 0; i < train_dataset.n_classes(); ++i) {
                ...
                for (int j = i + 1; j < train_dataset.n_classes(); ++j) {
                    ...
                    if (parser.param_cmd.nu * (n1 + n2) / 2 >
                        std::min(n1, n2)) {
        ...
        */
        /*
        DPCT1002:1: Special case error handling if-stmt was detected. You may
        need to rewrite this code.
        */
        /*
        DPCT1093:2: The "parser.gpu_id" may not be the best XPU device. Adjust
