 ...
 */
template <typename T> class SyncArray : public el::Loggable
{
  public:
    /**
     ...
     */
    explicit SyncArray(sycl::queue q, size_t count);
     ...
     */
    void copy_from(sycl::queue q, const T *source, size_t count);

    void copy_from(sycl::queue q, const SyncArray<T> &source);

    /**
     * set all elements to the given value. This method will set device data.
     * @param value
     */
    void mem_set(sycl::queue q, const T &value);

    /**
     * resize to a new size. This will also clear all data.
     * @param count
     */
    void resize(sycl::queue q, size_t count);
